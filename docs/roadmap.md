...
记录截至 3.0.0 已经实现的行为。未实现的原计划单独列出，不记入对应版本的交付范围。
...
| 3.0.0 | 无协议变更，服务端仍为 2.3.0。Android `versionCode` 57。设置收成两到三级。页面切换加大位移并带回弹。每日摘要使用当前选中的整理渠道。助手工具调用未做。见 [protocol-3.0.md](protocol-3.0.md) |

## 未实现

- 助手工具清单和固定格式回复尚未做，见 [plan-3.0.md](plan-3.0.md)。助手语音通话已在 2.4.0 从手机去掉。
