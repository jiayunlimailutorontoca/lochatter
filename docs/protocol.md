...
- [2.4](protocol-2.4.md)：无新增帧。手机不再发起或接听助手通话。服务端仍为 2.3.0。
- [2.5](protocol-2.5.md)：无新增帧。本机整理改用 Qwen，生成时逐字显示并可停止。服务端仍为 2.3.0。2.5.1 将聊天页右上角的语音通话、视频通话与助手入口移到加号面板，见 [protocol-2.5.1.md](protocol-2.5.1.md)。2.5.2 增加可选的 Qwen3.5 2B GGUF，在已打包的骁龙 Hexagon 上走 NPU，见 [protocol-2.5.2.md](protocol-2.5.2.md)。2.5.3 改为 NPU 或 GPU，不得再走 CPU，见 [protocol-2.5.3.md](protocol-2.5.3.md)。2.5.4 改为 MNN 在 GPU 上运行 Qwen3 0.6B、1.7B 或 4B，并允许用户自填云端接口，见 [protocol-2.5.4.md](protocol-2.5.4.md)。2.5.5 改为第一次整理时才载入模型，并在界面写明，载好后留在内存里，见 [protocol-2.5.5.md](protocol-2.5.5.md)。2.5.6 将整理的默认选项改为云端接口，并把它列在最前，见 [protocol-2.5.6.md](protocol-2.5.6.md)。3.0.0 将设置收成多级页面，加大页面过渡，并增加每日摘要。每日摘要使用当前选中的整理渠道。无新增帧。见 [protocol-3.0.md](protocol-3.0.md)。
