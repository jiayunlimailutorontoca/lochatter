...
- [2.3](protocol-2.3.md)：无新增帧。通话在中转接通后继续尝试直连。字幕与纪要只在本机处理。网页可播放语音和视频并下载文件。2.3.1 将纪要模型的下载与开关分开。2.3.2 将纪要模型的下载改为魔搭社区。2.3.3 保留通话字幕，并修正 Android 16 上接通即退出进程的录音拷贝。服务端仍为 2.3.0。
