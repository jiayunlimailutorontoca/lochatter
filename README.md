...
2.5.6 不修改协议与服务端。Android `versionCode` 为 56。整理默认改用云端接口，并排在选项第一位。本机三档仍可改选。详见 [protocol-2.5.6.md](docs/protocol-2.5.6.md)。

3.0.0 不修改协议与服务端。Android `versionCode` 为 57。设置收成多级页面。页面切换加大位移并带回弹。每日摘要与当前整理渠道相同。详见 [protocol-3.0.md](docs/protocol-3.0.md)。
